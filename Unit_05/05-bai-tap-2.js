...
// sử dụng trực tiếp array method
/*// 1. dùng map tạo normalizedRuns và chuẩn hoá dữ liệu
const normalizedRuns = rawRuns.map(run => ({
   id: run.id.trim().toUpperCase(),
   module: run.module.trim().toLowerCase(),
   status: run.status.trim().toLowerCase(),
   owner: run.owner.trim(),
   priority: run.priority,
   enabled: run.enabled}))
console.log(normalizedRuns);

// 2. dùng filter để tạo rerunList và chạy lại test theo yêu cầu
const rerunList = rawRuns.filter(run => 
   run.enabled === true &&
   run.id.trim() !== "" &&
   (run.status.trim().toLowerCase() === "fail" || run.status.trim().toLowerCase() === "flaky"))
console.log(rerunList);

// 3. dùng filter để tạo missingOwnerList và lấy các test theo yêu cầu
const   missingOwnerList = rawRuns.filter(run =>
   run.enabled === true &&
   run.id.trim() !== "" &&
   run.owner.trim() === ""
)
...
// 4. dùng find để tạo firstCriticalCase và thoã mãn các điều kiện
const    firstCriticalCase = rawRuns.find( run =>
   run.enabled === true &&
   run.id.trim() !== "" &&
   run.priority === 1 &&
   run.status.trim().toLowerCase() === "fail"
)
console.log(firstCriticalCase);*/

// áp dụng destructuring vào các array method
// 1. dùng map tạo normalizedRuns và chuẩn hoá dữ liệu
...
const rerunList = normalizedRuns.filter(({ enabled, id, status }) => (
   enabled === true &&
   id.trim() !== "" &&
   (status.trim().toLowerCase() === "fail" || status.trim().toLowerCase() === "flaky")
));
//console.log(rerunList);
...
return {
    ...
    rerunList,
    missingOwnerList,
    firstCriticalCase: firstCriticalCase || null
};
}
